...
def get_user_input():
    ...
    # Page 1
    data["id"] = input(
        Fore.GREEN + "\tEnter your Discord ID: (e.g. 408785106942164992)\n\t> "
    )
    cls()
...
def find_and_log_crystals(id, dirname):
    ...
    try:
        sorted_files = sorted(
            files,
            key=lambda x: (
                datetime.strptime(x[:-4], "%b %d")
                or (datetime.strptime(x[:-4], "%B %d"))
            ),
        )
    except:
        sorted_files = files
    ...
    for file in sorted_files:
        with open(
            os.path.join(dirname, file), "r", encoding="utf-8"
        ) as miner_json_file:
                ...
                if user_dict["id"] == id:
                    print(
                        (
                            ...
                            + " {0} "
                            + Style.NORMAL
                            + "with {1} crystals in '{2}'"
                        ).format(user_dict["tag"], user_dict["quantity"], file)
                    )
                    outfile.write(
